...
_._bindEvents = function() {
	...
	this.menu[0].querySelectorAll('#drawCircle')[0].onclick = function() {
	    ...
	    that.socket.emit('create', id, 'circle');
	...
	this.menu[0].querySelectorAll('#drawSquare')[0].onclick = function() {
	    ...
	    that.board.insertAdjacentHTML('beforeend', content);
	    that.socket.emit('create', id, 'square');
	    that.objs.push(new SVGObject('#svg' + id, that));
	    id++;
	}
	this.menu[0].querySelectorAll('#drawTriangle')[0].onclick = function() {
	    ...
	    that.board.insertAdjacentHTML('beforeend', content);
	    that.socket.emit('create', id, 'triangle');
	    that.objs.push(new SVGObject('#svg' + id, that));
	    id++;
	}
		...
		if(event.which == 27) {
			...
			for(var i = 0; i < that.objs.length; i++) {
				if(that.objs[i].isEditable) {
					...
					that.socket.emit('move', that.objs[i].id, 'left');
				}
			}
			...
			for(var i = 0; i < that.objs.length; i++) {
				if(that.objs[i].isEditable) {
					...
					that.socket.emit('move', that.objs[i].id, 'up');
				}	
			}
			...
			for(var i = 0; i < that.objs.length; i++) {
				if(that.objs[i].isEditable) {
					that.objs[i].update(2, 0, 0, 0);
					that.socket.emit('move', that.objs[i].id, 'right');
				}
			}
			...
			for(var i = 0; i < that.objs.length; i++) {
				if(that.objs[i].isEditable) {
					that.objs[i].update(0, 2, 0, 0);
					that.socket.emit('move', that.objs[i].id, 'down');
				}
			}
			...
			for(var i = 0; i < leng; i++) {
				if(that.objs[i].isEditable) {
					...
					if(index > -1) { 
						that.socket.emit('move', that.objs[i].id, 'delete');
	...
	this.socket.on('create', function(cid, type) {
		...
		that.board.insertAdjacentHTML('beforeend', content);
	    that.objs.push(new SVGObject('#svg' + cid, that));
	    id = cid + 1;
	})
	...
	this.socket.on('move', function(cid, type) {
		console.log('move :' + type + cid);
		if(type == 'left') {
			for(var i = 0; i < that.objs.length; i++) {
				if(that.objs[i].id == cid) {
					that.objs[i].update(-2, 0, 0, 0);
				}
			}
		} else if(type == 'right') {
	...
	this.socket.on('mouse', function(cid, dl, dt, dw, dh) {
		for(var i = 0; i < that.objs.length; i++) {
			if(that.objs[i].id == cid) {
				that.objs[i].update(dl, dt, dw, dh);
				that.objs[i].adjust();
				return;
			}
		}
...
_.onMouseMove = function(e) {
    ...
    this.par.socket.emit('mouse', this.id, dl, dt, dw, dh);
